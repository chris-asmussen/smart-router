...
from warden.config import config_home, resolve_config_path, writable_config_path


class ResolutionTests(unittest.TestCase):
    def test_config_home_uses_env_override(self):
        self.assertEqual(config_home(env={"WARDEN_HOME": "/x/y"}), pathlib.Path("/x/y"))

    def test_config_home_defaults_to_xdg(self):
        self.assertEqual(config_home(env={}), pathlib.Path("~/.config/warden").expanduser())

    def test_resolve_prefers_explicit_then_env(self):
        self.assertEqual(resolve_config_path("/a.json", env={"WARDEN_CONFIG": "/b.json"}), pathlib.Path("/a.json"))
        self.assertEqual(resolve_config_path(None, env={"WARDEN_CONFIG": "/b.json"}), pathlib.Path("/b.json"))

    def test_resolve_read_falls_back_to_cwd_default(self):
        self.assertEqual(resolve_config_path(None, env={}), pathlib.Path("warden.config.json"))
    ...
    def test_writable_never_falls_back_to_cwd(self):
        ...
        self.assertEqual(writable_config_path(None, env={"WARDEN_HOME": "/x"}), pathlib.Path("/x/config.json"))
        self.assertEqual(writable_config_path(None, env={}), pathlib.Path("~/.config/warden/config.json").expanduser())
...
class LoadConfigTests(unittest.TestCase):
    ...
    def test_explicit_path_is_loaded_and_defaults_filled(self):
        with tempfile.TemporaryDirectory() as tmp:
            cfg = pathlib.Path(tmp) / "warden.config.json"
    ...
    def test_env_var_is_honored(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            old = os.environ.get("WARDEN_CONFIG")
            os.environ["WARDEN_CONFIG"] = str(cfg)
            try:
                data = load_config()
            finally:
                if old is None:
                    del os.environ["WARDEN_CONFIG"]
                else:
                    os.environ["WARDEN_CONFIG"] = old
